    ...
    <table>
      ...
      <template v-for="order in filteredOrders">
        <tr class="row">
          ...
          <td>
            <div class="button" @click="toggle(order.id)" v-if="opened !== order.id">Edit</div>
          </td>
        </tr>
            ...
            <div class="orders__items">
              <table>
                ...
                <tr v-for="item in order.ordered_item">
                  ...
                  <td>
                    <div
                      class="button"
                      v-if="!item.is_picked_up && order.is_paid"
                      @click="setChangedItem(item.stock.id, order.id)"
                    >Picked up</div>
                  </td>
                </tr>
              </table>
              <div class="orders__items__buttons">
                <div
                  ...
                  @click="saveChanges(order.id)"
                  v-if="!order.is_picked_up"
                >Save</div>
                <router-link
        ...
        case "new":
          return this.orders.filter(order => {
            return (
              order.user.name
                .toLowerCase()
                .includes(this.orderSearch.toLowerCase()) && !order.is_picked_up
            );
          });
          break;
        default:
  ...
  methods: {
    ...
    setChangedItem(stockID, orderID) {
      ...
      let item = this.orders
        ...
        .ordered_item.find(item => item.stock.id === stockID);
    ...
    clearChanges() {
      for (let i = 0; i < this.changedData.length; i++) {
        let item = this.orders
          .find(order => order.id === this.changedData[i].order_id)
          .ordered_item.find(
            item => item.stock.id === this.changedData[i].stock_id
          );
